<?php
    ...
    // include("../uploadFile/popupUploadImage.php");
        ...
        if(@$save){
            ...
            if(@$error==''){
				...
				if(!isEmpty($entryId)){
                    ...
                    $row=fetchArray(runQuery("SELECT max(listorder) FROM `".$table."` WHERE `publication_id`='".@$publicationId."' AND `status`='1' "));
					if($row[0] != null){$listorder = $row[0]+1;}
					else{$listorder = 1;}
					$query="INSERT INTO `".$table."` (`publication_id` , `image` , `caption` , `datesubmit` , `status` , `listorder`) 
							VALUES( '".sanitizeInput($publicationId)."' , 
                ...
                if(runQuery($query)){
                    ...
                    $prompt=0;
					echo "<meta http-equiv='refresh' content='2;url=images.php?publicationid=".@$publicationId."'>";
                }else{
            ...
            <table cellpadding="0" cellspacing="0" class="prompt small">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <input onclick="window.location='images.php?publicationid=<?php echo @$publicationId; ?>'" class='submit' value='Cancel' type='Button' />
                    </td>
                </tr>
...
<script>
    ...
    $(document).ready(function(){
        ...
        //     $("#popupUploadImage").removeClass("hidden");
        // });
