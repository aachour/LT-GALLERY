...
<div id="middle" style="position:relative; width:100%; padding:50px 0px; background:#FFF;">
	...
	<?php
		...
		@$publicationId=sanitizeInput($publicationid);
		...
		if(@$doDelete){
			...
			echo "<meta http-equiv='refresh' content='2;url=".currentPage()."?publicationid=".$publicationId."'>";
			$list=0;
		}
		
		if(@$up){
			$previous=$listorder-1;
			runQuery("UPDATE `".$table."` SET `listorder`=".$listorder." WHERE `publication_id`='".$publicationId."' AND `listorder`=".$previous);
			runQuery("UPDATE `".$table."` SET `listorder`=".$previous." WHERE `id`=".$entryId);
		}
		
		if(@$down){
			$next=$listorder+1;
			runQuery("UPDATE `".$table."` SET `listorder`=".$listorder." WHERE `publication_id`='".$publicationId."' AND `listorder`=".$next);
		...
		if($list){
			...
			echo "<a href='imageEdit.php?publicationid=".$publicationId."'>
			...
			$query="SELECT * FROM `".$table."` WHERE `publication_id`='".@$publicationId."' AND `status`='1' ORDER BY `listorder` ASC ";
                ...
                while($row=fetchArray($result)){
					...
					echo '<tr>';
						...
						echo "<td>";
							...
							$row2=fetcharray(runQuery("select min(listorder) from `".$table."` WHERE `publication_id`='".@$publicationId."' AND `status`='1'"));
							if($listorder>$row2[0]){
								echo "<form action='images.php' method='post'>
									<input type='hidden' name='id' value='".$id."' />
									<input type='hidden' name='publicationid' value='".$publicationId."'/>
							...
							$row2=fetcharray(runQuery("select max(listorder) from `".$table."` WHERE `publication_id`='".@$publicationId."' AND `status`='1'"));
							if($listorder<$row2[0]){
								echo "<form action='images.php' method='post'>
									<input type='hidden' name='id' value='".$id."' />
									<input type='hidden' name='publicationid' value='".$publicationId."'/>
